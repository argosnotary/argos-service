...
Feature: Layout
  ...
  Background:
    ...
    * def supplyChainPath = '/api/supplychain/'+ supplyChainId
  ...
  Scenario: store layout with valid specifications should return a 200 and commit to auditlog
    * call read('create-layout.feature') {supplyChainId:#(supplyChain.response.id), layoutToBeSigned:#(validLayout), layoutSigner:#(pa1)}
    * def auditlog = call getAuditLogs { url: #(mongoUrl)}
    And match auditlog contains 'createOrUpdateLayout'
    And match auditlog contains 'supplyChainId'
    And match auditlog contains 'layout'
  ...
  Scenario: find layout with valid supplychainid should return a 200
    * def layoutResponse = call read('create-layout.feature') {supplyChainId:#(supplyChain.response.id), layoutToBeSigned:#(validLayout), layoutSigner:#(pa1)}
    Given path layoutPath
    When method GET
    Then status 200
  ...
  Scenario: update a layout should return a 200
    ...
    * def layoutResponse = call read('create-layout.feature') {supplyChainId:#(supplyChain.response.id), layoutToBeSigned:#(layoutToBeSigned), layoutSigner:#(pa1)}
    Given path layoutPath
    When method GET
    Then status 200
  ...
  Scenario: create ApprovalConfiguration should return a 201
    * def layoutResponse = call read('create-layout.feature') {supplyChainId:#(supplyChain.response.id), layoutToBeSigned:#(validLayout), layoutSigner:#(pa1)}
    * def response = call read('create-approval-config.feature') {supplyChainId:#(supplyChain.response.id)}
  ...
  Scenario: get approvalConfigurations should return a 200
    * def layoutResponse = call read('create-layout.feature') {supplyChainId:#(supplyChain.response.id), layoutToBeSigned:#(validLayout), layoutSigner:#(pa1)}
    * def createcall = call read('create-approval-config.feature') {supplyChainId:#(supplyChain.response.id)}
    Given path layoutPath+'/approvalconfig/'
    When method GET
    Then status 200
